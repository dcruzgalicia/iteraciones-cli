import { blockContent, hasClass } from '../_ast-utils.js';

/**
 * Transpiler de formato HTML: transforma Divs con clase .dictum a
 * <blockquote class="dictum"> con los bloques internos nativos.
 *
 * El Div.author anidado (autor de la cita) fluye como div.author dentro
 * del blockquote.
 *
 * Para sobrescribir en un proyecto, crear un archivo con el mismo
 * nombre en <proyecto>/transpilers/html/01-dictum.ts.
 */

export const type = 'ast' as const;

export async function transform(ast: Record<string, unknown>): Promise<Record<string, unknown>> {
  const blocks = ast.blocks as unknown[];
  if (!Array.isArray(blocks)) return ast;

  const newBlocks: unknown[] = [];
  for (const block of blocks) {
    if (
      typeof block === 'object' &&
      block !== null &&
      (block as Record<string, unknown>).t === 'Div' &&
      hasClass(block as Record<string, unknown>, 'dictum')
    ) {
      newBlocks.push({ t: 'RawBlock', c: ['html', '<blockquote class="dictum">'] });
      newBlocks.push(...blockContent(block as Record<string, unknown>));
      newBlocks.push({ t: 'RawBlock', c: ['html', '</blockquote>'] });
    } else {
      newBlocks.push(block);
    }
  }

  ast.blocks = newBlocks;
  return ast;
}
