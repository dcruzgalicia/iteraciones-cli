...
async function writeFakeBinary(dir: string, json: string): Promise<void> {
  const binDir = join(dir, '.iteraciones', 'bin');
  await mkdir(binDir, { recursive: true });
  await writeFile(join(dir, '.iteraciones', 'bin', 'iteraciones-pdfcheck'), `#!/bin/sh\ncat <<'EOF'\n${json}\nEOF\n`, 'utf8');
  await chmod(join(dir, '.iteraciones', 'bin', 'iteraciones-pdfcheck'), 0o755);
}
...
describe('runPdfxOutputValidation (fase final del build)', () => {
  ...
  it('omite la validación cuando 99-pdfx está desactivado en la config', async () => {
    await withTempDir(async (dir) => {
      ...
      let output = '';
      try {
        const result = await runPdfxOutputValidation(dir, join(dir, 'dist', 'files'), config, { allowBuild: false });
        output = stderrSpy.mock.calls.map((c) => String(c[0])).join('');
        expect(result).toEqual({ validated: 0, failed: 0 });
      } finally {
  ...
  it('muestra el fallo por PDF cuando el binario reporta errores de certificación', async () => {
    await withTempDir(async (dir) => {
      ...
      try {
        const result = await runPdfxOutputValidation(dir, join(dir, 'dist', 'files'), config, { allowBuild: false });
        output = stderrSpy.mock.calls.map((c) => String(c[0])).join('');
        expect(result).toEqual({ validated: 1, failed: 1 });
      } finally {
        stderrSpy.mockRestore();
      }
  ...
  it('sin fallos de certificación no emite warnings', async () => {
    await withTempDir(async (dir) => {
      await initPdfxProject(dir);
      await mkdir(join(dir, 'dist', 'files'), { recursive: true });
      await writeFile(join(dir, 'dist', 'files', 'doc.pdf'), '%PDF-1.4 fake', 'utf8');
      ...
      try {
        const result = await runPdfxOutputValidation(dir, join(dir, 'dist', 'files'), config, { allowBuild: false });
        output = stderrSpy.mock.calls.map((c) => String(c[0])).join('');
        expect(result).toEqual({ validated: 1, failed: 0 });
      } finally {
        stderrSpy.mockRestore();
      }
      expect(output).toBe('');
