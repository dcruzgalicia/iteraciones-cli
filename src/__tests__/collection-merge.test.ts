...
/**
 * #2437/#2452: format.markdown.merge
 * - false (default): dist/<collection>.md conserva type: collection y
 *   files[] (reescritos hacia el .md standalone de cada miembro), mantiene el
 *   body original de la collection; re-procesable e idempotente.
 * - true: dist/<collection>.md sale con type: file y el contenido fusionado,
 *   sin files[].
 * - Los miembros se construyen standalone en los dos modos (#2452).
 * - Las dos salidas de la collection son distintas por diseño.
...
describe.skipIf(!pandocOk)('format.markdown.merge y `iteraciones merge` (#2437)', () => {
  it('merge:false re-procesable, merge:true fusionado, y merge CLI determinista', async () => {
    await withTempDir(async (dir) => {
      ...
      // -- type: collection + files[] hacia el .md standalone de cada miembro
      ...
      expect(coleccion).toContain('documento-por-autora-a.md');
      expect(coleccion).toContain('otro-por-autora-b.md');
      expect(coleccion, 'el nombre de la fuente ya no aparece en files[]').not.toContain('  - doc.md');
      ...
      // -- cada miembro se construye standalone con su nombre-nuevo (#2452)
      const docExport = await Bun.file(join(dist, 'documento-por-autora-a.md')).text();
      expect(docExport).toContain('title: Documento');
      expect(docExport).toContain('Contenido de doc.');
      expect(existsSync(join(dist, 'otro-por-autora-b.md'))).toBe(true); // './other.md' normalizado
      expect(existsSync(join(dist, 'documento-por-autora-a.html'))).toBe(true);
      expect(existsSync(join(dist, 'otro-por-autora-b.html'))).toBe(true);
      // y no quedan copias con el nombre de la fuente
      expect(existsSync(join(dist, 'doc.md'))).toBe(false);
      ...
      for (const name of ['mi-coleccion.md', 'documento-por-autora-a.md', 'mi-coleccion.html']) {
      ...
      // -- un miembro modificado refresca su export y la collection que lo lista (#2452)
      await Bun.write(join(dirFalse, 'other.md'), OTHER.replace('Contenido de other.', 'Contenido modificado.'));
      await runBuild(dirFalse);
      expect(existsSync(join(dist, 'otro-por-autora-b.md'))).toBe(true);
      expect(await Bun.file(join(dist, 'otro-por-autora-b.md')).text(), 'el export del miembro se refresca').toContain('Contenido modificado.');
      const htmlDeLaColeccion = await Bun.file(join(dist, 'mi-coleccion.html')).text();
      expect(htmlDeLaColeccion, 'la collection se reconstruye al cambiar un miembro').toContain('Contenido modificado.');
      expect(existsSync(join(dist, 'other.md'))).toBe(false);
      expect(existsSync(join(dist, 'otro-por-autora-b.html'))).toBe(true);
      ...
      // el miembro se construye standalone también con merge:true: lo que
      // merge:true elimina es el files[] de la collection
      expect(existsSync(join(distTrue, 'documento-por-autora-a.md')), 'el miembro sigue siendo standalone').toBe(true);
      expect(existsSync(join(distTrue, 'doc.md'))).toBe(false);
      ...
      process.exitCode = 0;
      await runMerge(dirFalse, 'dist/files/documento-por-autora-a.md', { format: 'markdown', output: 'salida/nofu.md' });
      ...
      // -- al desactivar el markdown se retiran la collection y los .md de sus miembros
      ...
      expect(existsSync(join(dist, 'documento-por-autora-a.md')), 'export del miembro retirado').toBe(false);
      expect(existsSync(join(dist, 'otro-por-autora-b.md'))).toBe(false);
