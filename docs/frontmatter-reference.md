...
- `publishers-image`: imagen que sustituye al texto de `publishers` en la portada (solo LaTeX/PDF; si existe, el texto se ignora). Mismo mecanismo que `title-image`: ruta relativa al directorio del documento (o absoluta), centrada, con un **ancho máximo del 40%** de la caja de texto (si la imagen natural es más pequeña se muestra a tamaño natural; si es mayor, se escala conservando la proporción). Formatos soportados: `jpg`, `jpeg`, `png` y `pdf`.
