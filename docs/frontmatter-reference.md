...
Orden de páginas en el PDF (flujo KOMA-Script): `extratitle` (primera página) → portada → `uppertitleback` (arriba) y `lowertitleback` (abajo) en el reverso de la portada → `dedication` (página impar siguiente, bloque del 50% del ancho alineado a la derecha) → contenido → `colophon` (página par final, bloque del 75% del ancho centrado). El espacio vertical post-portada/índice sigue el mismo criterio que sin estas páginas: se aplica cuando al contenido le sigue un párrafo normal.
