...
| `latex/07-titlepages` | ast | campos multilinea de portada → LaTeX (extratitle, colofón, guardas…) |
| `latex/08-textsize` | ast | clases de tamaño (`.small`, `.footnotesize`, `.Large`…) → `{\small texto}` |
| `latex/09-quote-noindent` | ast | BlockQuote → `quote` + `\noindent` al primer párrafo siguiente |
| `latex/10-cjk` | ast | `Div.japanese/.chinese/.korean` → entorno CJKutf8 |
...
| `latex/14-textls` | ast | span/div con atributo `value` → `\textls[valor]{texto}` (microtype) |
...
- **Identificación (estricto 2001, issues #1964/#1967)**: el pipeline genera `PDF/X-1a:2001` (opción `x-1a1` del paquete `pdfx` — la desnuda `[x-1a]` emite 2003) e incluye la identificación XMP directa `pdfxid:GTS_PDFXVersion` mediante el template override `src/lib/resources/xmp/pdfx.xmp` (el paquete la lee con `\includexmp{pdfx}` buscando en la ruta de entrada; se copia al directorio de trabajo de latexmk y se expone vía `TEXINPUTS`). Así el PDF pasa la validación estricta sin warnings de identificación.
- **Resultado (decisión D2, issue #2162)**: con `99-pdfx` activo, un PDF que no certifica **hace fallar el build** (exit 1) con el detalle por PDF en el error: archivo, código, página y mensaje. El filter activo es la señal explícita de imprenta; para PDF sin certificación, desactivar `99-pdfx`. Los warnings de los PDFs que sí certifican quedan como advertencias sin bloquear. En éxito, el resumen confirma `✔ Validación PDF/X-1a: N PDFs certifican PDF/X-1a`. `doctor` lo reporta como check opcional (`iteraciones-pdfcheck disponible`).
