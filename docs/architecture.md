...
- **Resultado**: los fallos de certificación se reportan como advertencias en el resumen del build (código + página), sin romper el build (herramienta opcional). En éxito, el resumen confirma `✔ Validación PDF/X-1a: N PDFs certifican PDF/X-1a`. `doctor` lo reporta como check opcional (`iteraciones-pdfcheck disponible`).
